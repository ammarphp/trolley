...
// Archived v1 is local/read-only for new responses. Existing withdrawal credentials remain usable.
activeRun.sharing = false; state.collection = false;
...
function renderGuide() {
  ...
  <details><summary>What is actually recorded?</summary><p>Locally: the exact dilemma, generator version, seed, choice or skip, optional confidence and reason, button position, active viewing time, elapsed time, narrative stage, mode, filters, and response time. A maximum of 5,000 local responses is retained. Exports are available in Your record.</p><p>New sharing is closed in this archived edition. Earlier shared runs can still be withdrawn in preferences. Each new run has a fresh random identifier. No name, email, free text, or browser fingerprint is requested. The hosting and network providers still process normal connection metadata. The app does not claim that network access is anonymous.</p></details>
...
function openSettings() {
  ...
  dialog.innerHTML = `<div class="dialog-head"><div><p class="eyebrow">THIS PART REALLY IS UNDER YOUR CONTROL</p><h2 id="settings-title">Your terms.</h2></div><button class="icon-button close-dialog" aria-label="Close preferences">×</button></div>
    ...
    <div class="setting-row"><label for="collection">Archived run collection<small>New sharing is closed. Earlier shared runs can still be withdrawn below.</small></label><input class="toggle" id="collection" type="checkbox" disabled></div>
    ...
    <div class="setting-row restart-row"><label>A new run<small>Start with coffee and a fresh, unrelated run ID.</small></label><button class="secondary-button" id="restart">Start over</button></div><p class="small-copy">Notice ${CONSENT_VERSION}. New sharing is closed in this archived edition. Withdrawal remains available for earlier shared runs.</p>`;
...
async function syncPending() {
  return; // v2 isolates new collection; legacy exports and withdrawal remain available.
